...
# Staging Validation Plan & Test Matrix
...
| 8 | Navigate to **QR Publishing** and generate a QR code for the branch. | High-res PNG QR code generated with destination URL `/menu/<branch-slug>`. | [ ] |
| 9 | Navigate to **Audit Logs** and verify administrative operations. | Structured log entries exist for all mutations (`RESTAURANT_UPDATE`, `BRANCH_CREATE`, `MENU_CREATE`, `QR_CREATE`). | [ ] |
...
| **FS-11** | Database Unavailable | Stop PostgreSQL daemon, query `/api/health` | Returns HTTP 503 with `"database": {"status": "DOWN"}`. Zero database crash. | [ ] |
...
Collect baseline response times on the staging server:
